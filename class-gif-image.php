...
require_once ( dirname( __FILE__ ) . '/class-image-effect.php' );
	...
	class Gif_Image {
		...
		function __construct( $gif_data ) {
			...
			// Reject images that are too small to be valid, as well as pessimistically reject images
			// that will likely fatal by exceeding the memory limit before the request times out.
			if ( ( $this->max_len < 14 )
 			  || ( false !== $memory_limit && memory_get_usage() + 4 * $this->max_len > $memory_limit ) ) {
		...
		private function get_memory_limit() {
			// Low effort regex, as there is no pragmatic way of getting the memory limit
